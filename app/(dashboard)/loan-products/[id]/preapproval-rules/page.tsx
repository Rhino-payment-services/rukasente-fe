...
import {
  useCreateLoanPreApprovalRule,
  useDeleteLoanPreApprovalRule,
  useLoanProduct,
  useLoanProductPreApprovalRules,
} from "@/hooks/use-loan";
import {
  LOAN_RULE_OPERATORS,
  LOAN_RULE_TYPES,
...
export default function LoanProductPreApprovalRulesPage({
  params,
}: {
  ...
  const rulesQ = useLoanProductPreApprovalRules(id);
  const create = useCreateLoanPreApprovalRule(id);
  const remove = useDeleteLoanPreApprovalRule(id);
  ...
  const columns = useMemo<ColumnDef<LoanProductPreApprovalRule>[]>(
    () => [
      {
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <Button
            ...
            onClick={async () => {
              if (!window.confirm("Delete this pre-approval rule?")) return;
              try {
                await remove.mutateAsync(row.original.id);
                toast.success("Pre-approval rule deleted");
              } catch (err) {
  ...
  async function onSubmit(e: FormEvent) {
    ...
    try {
      ...
      setValue("");
      setDescription("");
      toast.success("Pre-approval rule added");
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-slate-900">Pre-Approval Rules</h1>
          <p className="mt-1 text-sm text-muted-foreground">
            Conditions for applying the pre-approved floor on{" "}
      ...
      <Card>
        <CardContent className="space-y-4 pt-4">
          ...
          <DataTable
            ...
            emptyMessage="No pre-approval rules configured for this product."
          />
        </CardContent>
      </Card>
    </div>
